...
public class PromotionAdatpter extends BaseSwipeAdapter {
	...
	private TextView tvTime, tvStatus, tvFrom, tvTo;
	...
	public void fillValues(final int position, final View v) {
		...
		System.out.println("time 1 : " + promotionTrip.getTime());
		System.out.println("time 2 : " + Utils.getDateTime());
		int i = promotionTrip.getTime().compareTo(Utils.getDateTime());
		...
		if (promotionTrip.getStatus().equalsIgnoreCase(
				PromotionTripStatus.CANCEL)) {
			tvStatus.setText(activity.getString(R.string.closed));
		}
		if (promotionTrip.getStatus()
				.equalsIgnoreCase(PromotionTripStatus.OPEN)) {
			tvStatus.setText(activity.getString(R.string.opening));
		}
